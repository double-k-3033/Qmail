...
import { base64ToUint8Array, decodeUint8ArrayTx, uint8ArrayToBase64 } from "@/Qubic/utils/utils";
...
import { balancesAtom } from "@/Qubic/store/balances";
...
export function QubicConnectProvider({ children }: QubicConnectProviderProps) {
  ...
  const toggleConnectModal = useCallback((): void => {
    ...
    switch (wallet.connectType) {
      ...
      case "walletconnect": {
        ...
        if (wallet?.connectType == "walletconnect") {
          signToastId = toast("Sign the transaction in your wallet", {
            icon: "🔑",
            duration: 10000,
          });
        }
        try {
          ...
          if (signToastId != null) toast.dismiss(signToastId);
        }
      }
